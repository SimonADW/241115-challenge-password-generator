	...
	const getPasswordStrength = (formValues: formValuesType) => {
		...
		for (const key in formValues) {
			if (
				...
				// Only score STRONG (4) if length i more than 7
				if (formValues.charLength < 8 && numberOfChecked >= 3) {
					numberOfChecked = 3;
				} else {
					numberOfChecked++;
				}
	...
	return (
		...
		>
			<section className={style.textInputSection}>
				<div className={style.passwordDisplay}>{generatedPassword}</div>
